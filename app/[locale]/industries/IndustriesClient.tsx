...
export default function IndustriesClient({
  ...
  const servicesT = require('@/lib/servicesTranslations').getServicesTranslations(locale);

  // Helper to extract image URL safely from Strapi JSON
  const getMediaUrl = (media: any) => {
    if (!media) return null;
    if (typeof media === "string") return media;
    let url = media.url || media.attributes?.url;
    if (!url && media?.data) {
    ...
    if (!url && Array.isArray(media)) {
      url = media[0]?.attributes?.url || media[0]?.url;
    }
  ...
  // Merge Strapi data with translations
  const localizedIndustries = useMemo(() => {
    const data = initialIndustries || [];

    return data.map((indRaw: any) => {
      const attrs = indRaw.attributes || indRaw;

      const ind = {
        id: indRaw.id || Math.random(),
        slug: attrs.slug || indRaw.slug || "",
        title: attrs.title || indRaw.title || "",
        summary: attrs.summary || indRaw.summary || "",
        heroImage:
          getMediaUrl(attrs.heroImage) ||
          indRaw.heroImage ||
          "/placeholder.jpg",
        featured:
          attrs.featured !== undefined
            ? attrs.featured
            : indRaw.featured || false,
        industryTag: attrs.industryTag || indRaw.industryTag || "",
        tags: attrs.tags || indRaw.tags || [],
        modules: attrs.modules || indRaw.modules || [],
      };

      // Generate camelCase key from slug (hotels-resorts -> hotelsResorts)
      let key = ind.slug.replace(/-([a-z])/g, (g: any) => g[1].toUpperCase());
  ...
  }, [initialIndustries, t.industries]);

  // State
  const [viewMode, setViewMode] = useState<"featured" | "all">("featured");
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedIndustryTag, setSelectedIndustryTag] = useState<string | null>(
    null,
  );
  const [selectedServiceTag, setSelectedServiceTag] = useState<string | null>(
    null,
  );
  ...
  // Dynamic Service List from Strapi or Fallback Config
  const definedServices = useMemo(() => {
    if (initialServices && initialServices.length > 0) {
      return initialServices.map((srv: any) => {
        ...
        return {
          label: trans?.title || attrs.title || srv.title || "",
          link: slug || ""
        };
      }).filter((s) => s.label);
    }
    return [
      ...
      { label: t.services.brandIdentity, link: "brand-identity" },
      { label: t.services.uiUxDesign, link: "ui-ux-design" },
      { label: t.services.motionDesign, link: "motion-design" },
      { label: t.services.seoMarketing, link: "seo-digital-marketing" },
      { label: t.services.ecommerceSolutions, link: "e-commerce-solutions" },
    ];
  }, [initialServices, servicesT.services, t.services]);

  // Derived Data for Filters
  const uniqueIndustryTags = Array.from(
    new Set(localizedIndustries.map((i) => i.industryTag)),
  );

  // Filtering Logic
  const filteredIndustries = useMemo(() => {
    return localizedIndustries.filter((industry: any) => {
      // 1. Featured Check
      ...
      if (searchQuery) {
        ...
        const matchesModules = industry.modules.some(
          (m: any) =>
      ...
      if (selectedServiceTag) {
        ...
        const serviceItem = definedServices.find(
          (s) => s.label === selectedServiceTag,
        );
        ...
        if (targetLink) {
          const hasService =
            industry.modules &&
            industry.modules.some((m: any) => m.serviceLink === targetLink);
  ...
  return (
    <main
      className="pt-32 min-h-screen bg-black text-white relative overflow-hidden"
      ref={containerRef}
    >
      {/* Background Glow */}
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-7xl pb-24 relative z-10">
        ...
        <div className="animate-header sticky top-24 z-50 mb-12">
          <div className="flex flex-wrap items-center gap-4 justify-center">
            ...
            <button
              onClick={() =>
                setViewMode(viewMode === "featured" ? "all" : "featured")
              }
              className={clsx(
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 bg-[#111] border border-white/10 rounded-2xl shadow-2xl overflow-hidden py-2 z-50 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  {uniqueIndustryTags.map((tag) => (
                    <button
                      key={tag as string}
                      onClick={() => {
                        setSelectedIndustryTag(tag as string);
                        setIndustryDropdownOpen(false);
                      }}
                      className={clsx(
                        "w-full text-left px-5 py-3 text-sm transition-colors",
                        selectedIndustryTag === tag
                          ? "text-[#00B5D9] bg-white/5"
                          : "text-white hover:text-[#00b6d9] hover:bg-white/5",
                      )}
                    >
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-72 bg-[#111] border border-white/10 rounded-2xl shadow-2xl overflow-hidden py-2 z-50 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  {definedServices.map((service) => (
                    <button
                      ...
                      className={clsx(
                        "w-full text-left px-5 py-3 text-sm transition-colors",
                        selectedServiceTag === service.label
                          ? "text-[#00B5D9] bg-white/5"
                          : "text-white hover:text-[#00b6d9] hover:bg-white/5",
                      )}
                    >
          ...
          {filteredIndustries.map((industry: any) => (
            ...
            >
              <article className="industry-card relative bg-zinc-900 border border-white/5 rounded-2xl overflow-hidden hover:border-[#00B5D9]/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#00B5D9]/10 h-full flex flex-col">
                {/* Image Area - Updated to use Standard IMG directly because of Strapi Domain restrictions */}
                ...
                <div className="p-6 flex flex-col grow">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6 mt-auto">
                    {industry.tags && Array.isArray(industry.tags)
                      ? industry.tags
                          .slice(0, 3)
                          .map((tag: any, idx: number) => (
                            <span
                              key={idx}
                              className="px-2 py-1 bg-white/5 text-gray-400 text-xs rounded-md border border-white/5"
                            >
                              {tag}
                            </span>
                          ))
