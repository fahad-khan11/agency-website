...
import {
  ...
  getIndustriesTranslations,
} from "@/lib/industriesTranslations";
...
export default function IndustryDetailClient({
  ...
  const attrs = initialIndustryData.attributes || initialIndustryData;
  const industryData = {
      title: attrs.title || "",
      summary: attrs.summary || "",
      industryTag: attrs.industryTag || "",
      heroImage: getMediaUrl(attrs.heroImage) || "/placeholder.jpg",
      featured: attrs.featured !== undefined ? attrs.featured : false,
      tags: attrs.tags || [],
      useCases: attrs.useCases || [],
      benefits: attrs.benefits || [],
      intro: attrs.intro || { painPoints: "", outcomes: "" },
      ...
      <section className="py-24 px-6 md:px-12 bg-zinc-900/20">
        <div className="container mx-auto">
          <div className="section-header mb-16 max-w-3xl">
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-[#00B5D9] mb-4">
              01. {t.page.theProblem}
            </h2>
          ...
          {industryData.useCases && industryData.useCases.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-list-container">
              {industryData.useCases.map((useCase: string, idx: number) => (
                <div
                  key={idx}
                  className="animate-list-item p-6 bg-zinc-900/50 rounded-2xl border border-white/5 hover:border-white/10 transition-colors"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            {industryData.benefits && industryData.benefits.length > 0 && (
              <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 animate-list-container">
                {industryData.benefits.map((benefit: string, idx: number) => (
                  <div
                    key={idx}
                    className="animate-list-item flex items-start gap-4 p-6 bg-[#00B5D9]/5 rounded-2xl border border-[#00B5D9]/10"
                  >
            ...
            <div className="bg-zinc-900/50 p-8 rounded-3xl border border-white/10 relative">
              ...
              <p className="text-gray-400 text-sm leading-relaxed mb-6">
                We don&apos;t just add features; we transform operations. By
                integrating these solutions, businesses achieve a new baseline
                of efficiency and customer satisfaction.
              </p>
              <Link
                href="/contact"
                className="text-[#00B5D9] text-sm font-bold uppercase tracking-wider flex items-center gap-2 hover:gap-3 transition-all"
              >
          ...
          <div className="container mx-auto">
            ...
            <div className="modules-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {industryData.modules.map((module: any, idx: number) => (
                <div
                  key={idx}
                  className="module-card group relative bg-zinc-950/50 backdrop-blur-sm border border-white/5 p-10 rounded-[2rem] hover:border-[#00B5D9]/40 transition-all duration-500 hover:-translate-y-2 flex flex-col h-full overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]"
                >
