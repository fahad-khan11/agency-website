...
export default function CaseStudiesClient({ initialCaseStudies, initialServices }: { initialCaseStudies: any[], initialServices?: any[] }) {
  ...
  const localizedStudies = useMemo(() => {
    const data = initialCaseStudies && initialCaseStudies.length > 0 ? initialCaseStudies : caseStudies;
    
    return data.map((studyRaw: any) => {
      const attrs = studyRaw.attributes || studyRaw;
      
      const study = {
          id: studyRaw.id || Math.random(),
          slug: attrs.slug || studyRaw.slug || "",
          title: attrs.title || studyRaw.title || "",
          category: attrs.category || studyRaw.category || "",
          year: attrs.year || studyRaw.year || "",
          client: attrs.client || studyRaw.client || "",
          tagline: attrs.tagline || studyRaw.tagline || "",
          heroImage: getMediaUrl(attrs.heroImage) || studyRaw.heroImage || '/placeholder.jpg',
          overview: extractText(attrs.overview) || studyRaw.overview || "",
          color: attrs.color || studyRaw.color || "#00b4d9",
          featured: attrs.featured !== undefined ? attrs.featured : (studyRaw.featured || false),
          services: Array.isArray(attrs.services) ? attrs.services : (studyRaw.services || []),
      };

      const key = study.slug.replace(/-([a-z])/g, (g:any) => g[1].toUpperCase());
  ...
  const uniqueServices = useMemo(() => {
    if (initialServices && initialServices.length > 0) {
      return initialServices.map((srv: any) => {
        ...
        return trans?.title || attrs.title || srv.title || "";
      }).filter(Boolean);
    }
    const servicesList = localizedStudies.flatMap(s => s.services);
    return Array.from(new Set(servicesList));
  }, [localizedStudies, initialServices, servicesT.services]);
