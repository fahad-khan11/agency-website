...
export default function CaseStudyDetailClient({ params, studyDataRaw, previousRaw, nextRaw }: { params: { slug: string; locale: string }; studyDataRaw: any; previousRaw: any; nextRaw: any }) {
    const { slug, locale } = params;
    const containerRef = useRef<HTMLDivElement>(null);
    const t = getCaseStudiesTranslations(locale || 'en');
    let study = getCaseStudyTranslation(locale || 'en', slug) as any;
    ...
    if (studyDataRaw) {
        const attrs = studyDataRaw.attributes || studyDataRaw;
        study = {
            ...study, 
            id: studyDataRaw.id || study?.id || 1,
            title: attrs.title || study?.title || "",
            category: attrs.category || study?.category || "",
            year: attrs.year || study?.year || "",
            client: attrs.client || study?.client || "",
            tagline: attrs.tagline || study?.tagline || "",
            heroImage: getMediaUrl(attrs.heroImage) || study?.heroImage || '/placeholder.jpg',
            detailImage1: getMediaUrl(attrs.detailImage1),
            detailImage2: getMediaUrl(attrs.detailImage2),
            overview: extractText(attrs.overview),
            challenge: extractText(attrs.challenge),
            solution: extractText(attrs.solution),
            color: attrs.color || study?.color || "#00b4d9",
            services: Array.isArray(attrs.services) ? attrs.services : study?.services || [],
            keyFeatures: Array.isArray(attrs.keyFeatures) ? attrs.keyFeatures : study?.keyFeatures || [],
            results: Array.isArray(attrs.results) ? attrs.results : study?.results || [],
            testimonial: attrs.testimonial || study?.testimonial || { quote: "", author: "", role: "" },
        };
    }

    const previous = previousRaw ? {
        slug: previousRaw.attributes?.slug || previousRaw.slug || "",
        title: previousRaw.attributes?.title || previousRaw.title || "",
        category: previousRaw.attributes?.category || previousRaw.category || ""
    } : previousRaw;

    const next = nextRaw ? {
        slug: nextRaw.attributes?.slug || nextRaw.slug || "",
        title: nextRaw.attributes?.title || nextRaw.title || "",
        category: nextRaw.attributes?.category || nextRaw.category || ""
    } : nextRaw;
                        ...
                        <div className="flex flex-col gap-6">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {study.services.map((service: string, idx: number) => (
                                        <span
                                            key={idx}
