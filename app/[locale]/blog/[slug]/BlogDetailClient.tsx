...
import { ArrowLeft, Clock, Calendar, Linkedin, Twitter, Link as LinkIcon } from "lucide-react";
import gsap from "gsap";
import { useGSAP } from "@gsap/react";

const STRAPI_URL = process.env.NEXT_PUBLIC_STRAPI_URL;

export default function BlogDetailClient({ post, locale, t, relatedPosts }: { post: any, locale: string, t: any, relatedPosts: any[] }) {
  ...
  // Handle content if it's JSON or string
  let htmlContent = "";
  if (typeof attrs.content === 'string') {
    htmlContent = attrs.content;
  } else if (attrs.content?.html) {
    htmlContent = attrs.content.html;
  }
  ...
  return (
    <article className="pt-32 min-h-screen bg-black text-white selection:bg-[#00B5D9] selection:text-black" ref={containerRef}>
      <div className="fixed top-0 left-0 w-full h-1 bg-white/10 z-50">
        <div className="h-full bg-[#00B5D9] w-[15%]" id="progress-bar" />
      </div>

      <div className="container mx-auto px-6 md:px-12 max-w-5xl relative z-10">
        <div className="mb-12 animate-in">
          <Link href={`/${locale}/blog`} className="inline-flex items-center text-sm font-medium text-gray-500 hover:text-[#00B5D9] transition-colors group">
          ...
          <div className="flex flex-col md:flex-row items-center md:justify-between gap-6 border-y border-white/10 py-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-[#00B5D9] to-blue-600 flex items-center justify-center text-xl font-bold text-black border-2 border-black">
                {attrs.author?.charAt(0) || "A"}
              </div>
              <div className="text-left">
                <p className="text-sm font-bold text-white">{attrs.author}</p>
                <div className="flex items-center gap-3 text-xs text-gray-400 font-mono mt-1">
                  <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {attrs.date}</span>
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {attrs.readTime}</span>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wider mr-2">{t.page?.share || "Share:"}</span>
            ...
            <div className="mt-16 pt-8 border-t border-white/10">
              <div className="flex items-center gap-4 p-6 rounded-3xl bg-zinc-900/30 border border-white/5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-lg font-bold text-white">{attrs.author}</p>
                  <div className="flex items-center gap-4 text-sm text-gray-400 font-mono mt-2">
                    <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {attrs.date}</span>
                    <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> {attrs.readTime}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-3xl bg-zinc-900/30 border border-white/5">
              ...
              <ul className="space-y-6">
                {relatedPosts.map(related => {
                  const rAttrs = related.attributes || related;
                  return (
                    <li key={rAttrs.slug}>
                      <Link href={`/${locale}/blog/${rAttrs.slug}`} className="group block">
                        <span className="text-[10px] uppercase font-bold text-[#00B5D9] mb-1 block">
                          {Array.isArray(rAttrs.categories) ? rAttrs.categories[0] : ""}
                        </span>
