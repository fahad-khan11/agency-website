...
export default function BlogPage() {
  ...
  const localizedPosts = blogPosts.map(post => {
    ...
    const trans = (t.posts as any)[key];
    return {
      ...post,
      title: trans?.title || post.title,
      excerpt: trans?.excerpt || post.excerpt,
      categories: trans?.categories || post.categories,
      tags: trans?.tags || post.tags,
      author: trans?.author || post.author,
      date: trans?.date || post.date,
      readTime: trans?.readTime || post.readTime
    };
  });
