...
export default function ProjectDetailClient({ project: rawProject, nextProject: rawNextProject, locale }: { project: any, nextProject: any, locale: string }) {
  const container = useRef(null);

  // Unpack Strapi v4 attributes if present
  const project = rawProject?.attributes || rawProject;
  let nextProject = rawNextProject?.attributes || rawNextProject;
    ...
    <ReactLenis root>
      <main ref={container} className="bg-black text-white min-h-screen">
        ...
        <header className="pt-40 md:pt-60 pb-12 px-6 md:px-12 container mx-auto relative z-10">
          ...
          <p className="hero-text text-xl md:text-3xl text-gray-400 max-w-3xl leading-relaxed whitespace-pre-line">
            {extractText(project.description)}
          </p>
        </header>
        ...
        <div className="hero-container w-full h-[80vh] relative overflow-hidden mb-24 z-0">
          ...
          <div className="hero-media w-full h-[120%] relative -top-[10%]">
            <MediaDisplay
              src={getMediaUrl(project.heroVideo) || getMediaUrl(project.heroImage)}
        ...
        <section className="container mx-auto px-6 md:px-12 py-24 border-t border-gray-900 grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="text-lg text-gray-300">
              {project.services?.map((s: string) => <li key={s}>{s}</li>) || <li>Design & Dev</li>}
            </ul>
          </div>
          <div>
        ...
        <section className="container mx-auto px-6 md:px-12 py-32 grid md:grid-cols-12 gap-12 bg-neutral-900/20 rounded-3xl mx-6 mb-20">
          <div className="md:col-span-5">
            ...
            <p className="text-gray-400 leading-relaxed text-lg whitespace-pre-line">
              {extractText(project.challenge) || "Every project starts with a unique problem. We identify the core constraints and opportunities to define a clear path forward."}
            </p>
          </div>
          <div className="md:col-span-2 hidden md:block" />
          <div className="md:col-span-5">
            <h2 className="text-3xl font-display font-semibold mb-6 text-[#00b4d9]">The Solution</h2>
            <p className="text-gray-200 leading-relaxed text-xl whitespace-pre-line">
              {extractText(project.solution) || "Our approach combines strategic insight with creative execution. We crafted a solution that not only meets the requirements but elevates the brand experience."}
            </p>
          </div>
        </section>
        ...
        <section className="relative w-full">
          {(project.images?.data || project.images || []).map((mediaObj: any, index: number) => {
            const mediaSrc = getMediaUrl(mediaObj);
        ...
        {/* Next Project */}
        <Link href={`/${locale}/projects/${nextProject.slug}`} className="block group relative overflow-hidden h-[80vh] bg-neutral-900 border-t border-white/10 mt-20">
          ...
          <div className="absolute inset-0 opacity-30 group-hover:opacity-60 transition-opacity duration-700 saturate-0 group-hover:saturate-100">
            <MediaDisplay
              src={getMediaUrl(nextProject.heroVideo) || getMediaUrl(nextProject.heroImage)}
