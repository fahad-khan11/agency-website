...
export default function ServiceDetailClient({ slug, locale, initialServiceData }: { slug: string, locale: string, initialServiceData?: any }) {
  // Get merged translated static data
  const localService = getServiceTranslation(locale, slug);
  const t = getServicesTranslations(locale);
  
  // Extract Strapi data
  const attrs = initialServiceData ? (initialServiceData.attributes || initialServiceData) : null;
  ...
  const service = {
    title: attrs?.title || localService?.title || "",
    category: attrs?.category || localService?.category || "",
    tagline: attrs?.tagline || localService?.tagline || "",
    description: attrs?.description || localService?.description || "",
    overview: attrs?.overview || localService?.overview || "",
    heroImage: getMediaUrl(attrs?.heroImage) || localService?.heroImage || null,
    heroVideo: getMediaUrl(attrs?.heroVideo) || localService?.heroVideo || null,
    features: attrs?.features || localService?.features || [],
    technologies: attrs?.technologies || localService?.technologies || [],
    deliverables: attrs?.deliverables || localService?.deliverables || [],
    process: attrs?.process || localService?.process || [],
    results: attrs?.results || localService?.results || [],
    caseStudy: attrs?.caseStudy || localService?.caseStudy || { client: "", challenge: "", solution: "", results: "" },
    icon: localService?.icon || "" // Keep static icon
  };
  
  const container = useRef(null);

  if (!attrs && !localService) {
    notFound();
  }
          ...
          <div className="container mx-auto px-6 md:px-12 max-w-6xl">
            ...
            <section className="content-section mb-24">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {service.deliverables.map((deliverable: any, i: number) => (
            ...
            <section className="content-section mb-24">
              ...
              <div className="grid md:grid-cols-3 gap-8 mb-16">
                {service.results.map((result: any, i: number) => (
                  <div key={i} className="group relative">
                    {/* Animated Background Glow */}
                    <div className="absolute inset-0 bg-gradient-to-br from-[#00B5D9]/0 to-[#00B5D9]/0 rounded-xl opacity-0 group-hover:from-[#00B5D9]/10 group-hover:to-[#00B5D9]/5 group-hover:opacity-100 transition-all duration-500 blur-xl" />
