...
export default function ServicesClient({ initialServices }: { initialServices?: any[] }) {
  ...
  // Extract Strapi data
  const strapiServices = initialServices && initialServices.length > 0
    ? initialServices.map((srv: any) => {
        ...
        const trans = key ? (t.services as any)[key] : null;
  ...
  // Merge static data with Strapi or fallback to translations for the grid
  const localizedServices = strapiServices
    ? strapiServices.map((strapiSrv: any) => {
        // Fallback to static for icon if needed
        const staticSrv = services.find((s) => s.slug === strapiSrv.slug);
        return {
          ...strapiSrv,
          icon: staticSrv?.icon || "",
        };
